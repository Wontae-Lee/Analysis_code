...
class Trainer(object):
    ...
    def train(self):
        # train이 가장 처음 할 멤버함수이다.
        # 왜? val test 모두 train이 있어야 가능하기 때문이다.
        # 내가 이 코드를 싫어하는 이유가 이것이다. cpp를 wrapper로 이용하는 것은 좋다.
        # 하지만 그말은 python 인터페이스 안의 내부동장으로 cpp를 이용해야하지
        # cpp코드로 이미 만들어진 데이터 상에서 이름은 train 이라 적고
        # 실제로는 이미 다 학습이 끝난 데이터와 weights를 index로 순회하는 것에 불가하다.
